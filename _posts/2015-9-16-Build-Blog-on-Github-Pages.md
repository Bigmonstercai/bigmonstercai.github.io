...
&emsp;&emsp;在项目库根目录下创建index.html或index.md来作为整个站点的首页，具体格式与日志的格式没有什么区别。通常在首页使用<code><a>{</a>% for post in site.posts %<a>}</a></code>来对所有日志进行遍历，然后调用每一篇日志的标题、摘要等信息显示在主页上。
